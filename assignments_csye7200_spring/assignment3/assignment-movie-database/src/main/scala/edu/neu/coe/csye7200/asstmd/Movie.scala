...
object Movie extends App {
  ...
  trait IngestibleMovie extends Ingestible[Movie] {
    ...
    def fromString(w: String): Try[Movie] = Try(Movie(w.split(",").toSeq)) // TO BE IMPLEMENTED 11 points //TODO: implement this line
  }
  ...
  def elements(list: Seq[String], indices: Int*): List[String] = {
    ...
    for (index <- indices) {
      x += list(index)
    }// TO BE IMPLEMENTED 6 points //TODO: implement this line
    x.toList
  }
...
object Rating {
  ...
  def apply(s: String): Rating = s match {
    case rRating(tag, _, null) => this.apply(tag, None)
    case rRating(tag, _, age) => this.apply(tag, Some(age.toInt))
    case _ => throw new Exception(s"No such rating category: $s")
    // TO BE IMPLEMENTED 13 points //TODO: implement this line
  }
}
